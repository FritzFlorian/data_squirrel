...
fn ignore_rules() {
    ...
    let mut data_store = FSInteraction::create_with_fs("", fs.clone()).unwrap();

    // No ignore rules, everything should be there
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-2.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir/test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir/test-2.txt")));

    // Add temporary ignore rule for file-1.txt
    data_store.add_ignore_rule("**/test-1.txt", false).unwrap();
    assert!(data_store.is_ignored(&RelativePath::from_path("test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-2.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir")));
    assert!(data_store.is_ignored(&RelativePath::from_path("dir/test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir/test-2.txt")));
    //...also try the rules for an actual scan process.
    fs.create_file("test-1.txt").unwrap();
    fs.create_file("test-2.txt").unwrap();
    fs.create_dir("dir", false).unwrap();
    fs.create_file("dir/test-1.txt").unwrap();
    let root_items = data_store.index(&RelativePath::from_path("")).unwrap();
    assert_eq!(root_items.len(), 3);
    assert!(root_items
        .iter()
        .any(|item| item.relative_path.name() == "test-1.txt" && item.ignored));
    assert!(root_items
        .iter()
        .any(|item| item.relative_path.name() == "test-2.txt" && !item.ignored));
    let dir_items = data_store.index(&RelativePath::from_path("dir")).unwrap();
    assert_eq!(dir_items.len(), 1);
    assert!(dir_items.iter().any(|item| item.ignored));

    // Add permanent ignore rule for sub-dir
    data_store.add_ignore_rule("/dir", true).unwrap();
    assert!(data_store.is_ignored(&RelativePath::from_path("test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-2.txt")));
    assert!(data_store.is_ignored(&RelativePath::from_path("dir")));
    assert!(data_store.is_ignored(&RelativePath::from_path("dir/test-1.txt")));
    ...
    // Re-open the store, be sure it keeps permanent rules
    drop(data_store);
    let data_store = FSInteraction::open_with_fs("", fs.clone()).unwrap();
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-1.txt")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("test-2.txt")));
    assert!(data_store.is_ignored(&RelativePath::from_path("dir")));
    assert!(!data_store.is_ignored(&RelativePath::from_path("dir/test-1.txt")));
